...
/*!
    ...
    Данный класс создан для регулирования игрового процесса

    Сложность:
        goo
    * Проверка съела ли змейка  еду : O(1)
    * Увеличение змейки: O(1) (Только если змейка съела еду)
        checker
    * Проверка на столкновение со стенами: O(1)
    * Проверка на столкновение головы змейки с остальным туловищем: O(длина змеи)
        check_food
    * Проверка еды на правильное расположение на игровом поле: O(длина змеи)


    Я использую для хранения координат змейки std::list.
    У листа есть преимущество перед обычным массивом в том, что у него быстрое добавление и удаление в начале/конце листа.
    Единственный минус листа это долгое обращение к серединным  элементам.
    В игре часто требуется добавлять элементы в начало и удалять элементы из конца, а обращение по индексу не требуется.
*/
